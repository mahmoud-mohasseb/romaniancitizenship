  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-slate-100 flex flex-col">
      ...
      <main className={`flex-1 w-full max-w-4xl mx-auto px-4 py-6 space-y-6 ${isRtl ? 'rtl' : 'ltr'}`} dir={isRtl ? 'rtl' : 'ltr'}>
        {/* Header Title Banner */}
        <div className="bg-slate-800/90 rounded-2xl p-6 border border-slate-700/80 shadow-xl space-y-3 text-center">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder={appLang === 'ar' ? 'ابحث عن حرف أو كلمة رومانية...' : 'Search letter or word...'}
              className="w-full bg-slate-800/80 border border-slate-700/80 text-white placeholder-slate-500 rounded-2xl py-3 px-11 text-xs sm:text-sm focus:outline-none focus:border-rose-500"
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredData.map((item) => {
            ...
            return (
              <div 
                key={item.id}
                className={`bg-slate-800/90 rounded-2xl p-5 border shadow-xl flex flex-col justify-between space-y-4 transition-all hover:border-slate-500 ${
                  isSpecial ? 'border-amber-500/50 bg-gradient-to-br from-slate-800 to-amber-950/20' : 'border-slate-700/80'
                }`}
              >
                {/* Header Badge */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3 space-x-reverse">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-white">
                        {item.name_ro} <span className="text-xs font-normal text-slate-400">{item.pronunciation_ro}</span>
                      </h3>
                      <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-md mt-0.5 ${
                        isSpecial ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-slate-700 text-slate-300'
                      }`}>
                        {item.category === 'special_vowels' || item.category === 'special_consonants' ? 'حرف روماني خاص' : 'حرف أساسي'}
                      </span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="p-3 bg-rose-500/15 hover:bg-rose-500/25 text-rose-400 rounded-xl transition-colors flex items-center space-x-1.5 space-x-reverse"
                ...
                {/* Pronunciation Notes */}
                <div className="bg-slate-900/90 rounded-xl p-3 border border-slate-700/80 space-y-1.5 text-xs">
                  <p className="font-bold text-amber-400 flex items-center space-x-1 space-x-reverse">
                    <Info className="w-3.5 h-3.5 shrink-0" />
                    <span>🇸🇦 النطق بالعربية:</span>
                  </p>
                  <p className="text-slate-200 text-right leading-relaxed font-semibold">
                    {item.pronunciation_ar}
                  </p>

                  <div className="h-px bg-slate-800 w-full" />

                  <p className="text-slate-400 text-left font-medium">
                    🇬🇧 <span className="font-bold text-emerald-400">English:</span> {item.pronunciation_en}
                  </p>
                </div>

                {/* Example Word & Image */}
                <div className="flex items-center justify-between bg-slate-900/60 rounded-xl p-3 border border-slate-800">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-rose-400 font-bold">
                      🇸🇦 {item.example_translation_ar} <span className="text-slate-400 font-normal">| 🇬🇧 {item.example_translation_en}</span>
                    </p>
                  </div>
                ...
                {/* Example Sentence */}
                <div className="space-y-1 text-xs pt-1 border-t border-slate-700/60">
                  <span className="text-[10px] font-bold text-slate-400 block">{strings.exampleSentence}</span>
                  <p className="font-bold text-emerald-400">{item.example_sentence_ro}</p>
                  <p className="text-slate-300 text-right">🇸🇦 {item.example_sentence_ar}</p>
                  <p className="text-slate-400">🇬🇧 {item.example_sentence_en}</p>
                </div>
              </div>
            );
          })}
