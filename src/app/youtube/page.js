...
function YoutubeContent() {
  ...
  // Track currently playing video per channel or active video
  ...
  // Quiz State for active video
  const [userQuizAnswer, setUserQuizAnswer] = useState(null);
  const [quizScore, setQuizScore] = useState(0);

  const categories = [
    { id: 'all', label_ar: 'جميع القنوات والمقاطع (All Channels & Videos)', label_en: 'All Channels & Videos', label_ro: 'Toate Canalele & Videoclipurile' },
      ...
      <main className={`flex-1 w-full max-w-4xl mx-auto px-4 py-6 space-y-6 ${
        ...
        <div className={`rounded-2xl p-6 border shadow-xl space-y-3 text-center ${isDark ? 'bg-slate-800/90 border-slate-700/80' : 'bg-white border-slate-200'}`}>
          <span className="inline-block px-4 py-1.5 rounded-full text-xs font-bold bg-rose-500/20 text-rose-400 border border-rose-500/30">
            📺 {strings.youtubeTitle} & Video Quizzes
          </span>
          <h1 className="text-2xl sm:text-3xl font-extrabold">
            {strings.youtubeTitle}
          </h1>
          <p className="text-xs sm:text-sm text-theme-sub max-w-xl mx-auto leading-relaxed">
            شاهد مقاطع الفيديو التعليمية المصنفة لكل قناة، اقرأ النص المترجم، واختبر فهمك مع اختبار الفيديو التفاعلي!
          </p>
        </div>

        {/* ACTIVE VIDEO PLAYER & TRANSCRIPT QUIZ SECTION */}
        {selectedVideo && activeChannel && (
            ...
            <div className="p-4 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-700/60 pb-3">
                <div>
                  <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wider block">
                    📺 {activeChannel.channel_name} ({activeChannel.subscribers})
                  </span>
                  <h2 className="text-lg font-black text-theme-main mt-0.5">
                    {selectedVideo.title_ro}
                  </h2>
                ...
                <a
                  ...
                  className="py-2 px-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl text-xs flex items-center space-x-1 space-x-reverse shadow transition-colors shrink-0"
                >
                  <span>افتح القناة على يوتيوب 📺</span>
              ...
              <div className={`rounded-xl p-4 border space-y-2 text-xs ${
                ...
                <div className="flex items-center space-x-1.5 space-x-reverse text-rose-500 font-bold">
                  ...
                  <span>نص الفيديو والترجمة (Video Transcript):</span>
                </div>
                ...
                <div className={`rounded-xl p-4 border-2 space-y-3 ${
                  ...
                  <div className="flex items-center justify-between border-b border-amber-500/30 pb-2">
                    <span className="text-xs font-black text-amber-500 flex items-center space-x-1.5 space-x-reverse">
                      ...
                      <span>اختبار فهم الفيديو (Video Transcript Quiz 🎮)</span>
                    </span>
        ...
        {/* CHANNELS & VIDEOS DIRECTORY GRID */}
        <div className="space-y-6">
          {filteredChannels.map((channel) => (
            ...
            >
              ...
              {/* Channel Videos Selector List */}
              <div className="px-5 pb-5 space-y-2">
                <p className="text-xs font-black text-theme-main block">
                  🎬 المقاطع المتاحة للتشغيل والاختبار من قناة ({channel.channel_name}):
                </p>
