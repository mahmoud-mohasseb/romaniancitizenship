...
function AIContent() {
  ...
  return (
    <div className={`space-y-4 flex flex-col h-[90vh] ${isRtl ? 'rtl' : 'ltr'}`} dir={isRtl ? 'rtl' : 'ltr'}>
      {/* Top Header */}
      <div className="flex items-center justify-between bg-slate-800/80 backdrop-blur-md p-3 rounded-2xl border border-slate-700/60 shrink-0">
        ...
        <div className="text-center">
          <h1 className="text-base font-bold text-white flex items-center space-x-1.5 space-x-reverse justify-center">
            <Sparkles className="w-4 h-4 text-amber-400" />
            <span>AI Romanian Tutor</span>
          </h1>
          <p className="text-[10px] text-slate-400">Ollama API + Embedded Knowledge Engine</p>
        </div>
        <button
          onClick={() => setShowConfig(!showConfig)}
          className="p-2 bg-slate-900 text-emerald-400 rounded-xl border border-slate-700 hover:text-white"
        >
